...
class WarrantiesController extends AdminController
{
	/**
...
//	}
                            ...
                            if (filter_input(INPUT_POST, 'submit')) {
                                ...
                                foreach (CommonProcess::getListTeeth() as $teeth) {
                                    if (isset($_POST['teeth'][$index])
                                        ...
                                        OneMany::insertOne($model->id, $index, OneMany::TYPE_WARRANTY_TEETH);
                                    }
                                    $index++;
                                }
                            }
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Warranties']))
		{
			...
			if($model->save()) {
                            if (filter_input(INPUT_POST, 'submit')) {
                                // Remove old record
                                OneMany::deleteAllOldRecords($model->id, OneMany::TYPE_WARRANTY_TEETH);
                                $index = 0;
                                foreach (CommonProcess::getListTeeth() as $teeth) {
                                    if (isset($_POST['teeth'][$index])
                                            && ($_POST['teeth'][$index] == DomainConst::CHECKBOX_STATUS_CHECKED)) {
	 ...
	 * @return Warranties the loaded model
