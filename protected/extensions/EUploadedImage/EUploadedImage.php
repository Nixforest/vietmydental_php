...
class EUploadedImage extends CComponent
{
	...
	public function saveAs($file)
	{
		if($this->_error==UPLOAD_ERR_OK)
		{
			...
			if ($this->saveImage($file)) {
				...
				if ($this->thumb) {
					...
					$thumb_dir = isset($this->thumb['dir']) ? DIRECTORY_SEPARATOR.$this->thumb['dir'].DIRECTORY_SEPARATOR : NULL;
					...
					//if ($this->saveImage(substr_replace($file, $thumb_dir.$thumb_prefix, strrpos($file, DIRECTORY_SEPARATOR) + 1, 0)))
                                        if ($this->saveImage( dirname( $file ). $thumb_dir . $thumb_prefix . basename( $file ) ) )
                                            return true;
                                 } else return true;
         ...
         * 
         */
	public function saveAsbase($file)
	{
		if($this->_error==UPLOAD_ERR_OK)
		{
                        // create the thumbnail
                        if ($this->base) {
                                // create the base image requested dimensions
                                $base_max_width = isset($this->base['maxWidth_base']) ? $this->base['maxWidth_base'] : false;
                                $base_max_height = isset($this->base['maxHeight_base']) ? $this->base['maxHeight_base'] : false;
                                //print $base_max_height;exit;
                                $this->imagebasePrepare($base_max_width, $base_max_height);

                                // get the thumb folder
                                $base_dir = isset($this->base['dir']) ? DIRECTORY_SEPARATOR.$this->base['dir'].DIRECTORY_SEPARATOR : NULL;

                                // get the thumb prefix
                                $base_prefix = isset($this->base['prefix']) ? $this->base['prefix'] : $this->_base_prefix;
                                // save the thumbnail
                                //if ($this->saveImage(substr_replace($file, $thumb_dir.$thumb_prefix, strrpos($file, DIRECTORY_SEPARATOR) + 1, 0)))
                                if ($this->saveImage( dirname( $file ). $base_dir . $base_prefix . basename( $file ) ) )
                                    return true;
                            } else return true;
	...
	private function imagePrepare($maxWidth, $maxHeight)
	{
        ...
        private function imagebasePrepare($maxWidth, $maxHeight)
	{
		if (is_int($maxWidth) && $this->__width > $maxWidth) {
			$this->resizeImage($maxWidth, ($maxWidth * $this->__height / $this->__width));
		}
		if (is_int($maxHeight) && $this->__height > $maxHeight) {
			$this->resizeImage(($maxHeight * $this->__width / $this->__height), $maxHeight);
		}
	}
